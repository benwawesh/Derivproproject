...
import { getContractTypeDisplay as _getContractTypeDisplay } from 'Constants/contract';

// ContractCard.Header calls getContractTypeDisplay(type, { isHighLow, showMainTitle })
// but our local version expects (type, is_high_low_bool). This wrapper bridges the gap.
const getContractTypeDisplay = (type: string, options?: boolean | Record<string, unknown>) => {
    const is_high_low = typeof options === 'boolean' ? options : !!(options as any)?.isHighLow;
    return _getContractTypeDisplay(type as any, is_high_low);
};
