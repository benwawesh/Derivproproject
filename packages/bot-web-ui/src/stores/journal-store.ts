...
export default class JournalStore {
    ...
    pushMessage(
        ...
        if (loginid) {
            ...
            const is_funded_mode = (window as any).__dpa_funded_active || (window as any).__dpa_user_chose === 'funded';
            extra.current_currency = is_funded_mode
                ? 'Funded'
                : current_account?.is_virtual
                  ? 'Demo'
                  : current_account?.title;
    ...
    registerReactions() {
        ...
        // Log journal message immediately when user switches funded mode on/off
        const onFundedActivated = () => {
            this.pushMessage(LogTypes.WELCOME, MessageTypes.SUCCESS, 'journal__text', { current_currency: 'Funded' });
        };
        const onFundedDeactivated = () => {
            const { client } = this.core;
            const current_account = client.account_list?.find((a: any) => a.loginid === client.loginid);
            const label = current_account?.is_virtual ? 'Demo' : (current_account?.title ?? 'Real');
            this.pushMessage(LogTypes.WELCOME, MessageTypes.SUCCESS, 'journal__text', { current_currency: label });
        };
        window.addEventListener('dpa_funded_challenge_activated', onFundedActivated);
        ...
        return () => {
            ...
            window.removeEventListener('dpa_funded_challenge_activated', onFundedActivated);
            window.removeEventListener('dpa_funded_deactivated', onFundedDeactivated);
        };
    }
}
