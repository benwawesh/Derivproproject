...
export default class TransactionsStore {
    ...
    pushTransaction(data: TContractInfo) {
        ...
        if (same_contract_index === -1) {
            ...
            const prev = this.elements[current_account]?.[same_contract_index];
            const was_completed = prev && typeof prev.data === 'object' && (prev.data as TContractInfo).is_completed;
            ...
            // Notify core to record this trade to Supabase (only once, only real trades)
            const is_funded_trade = !!(data as any)._is_funded;
            if (is_completed && !was_completed && !is_funded_trade) {
                window.dispatchEvent(
                    new CustomEvent('dpa_real_trade_completed', {
                        detail: {
                            loginid: current_account,
                            contract_id: String((data as TContractInfo).contract_id ?? ''),
                            market: (data as TContractInfo).underlying ?? '',
                            trade_type: (data as TContractInfo).contract_type ?? '',
                            stake: (data as TContractInfo).buy_price ?? 0,
                            payout: (data as TContractInfo).payout ?? 0,
                            profit: (data as TContractInfo).profit ?? 0,
                            is_win: ((data as TContractInfo).profit ?? 0) > 0,
                        },
                    })
                );
            }
