        ...
        <div className="mm-differs__dashboard">
            ...
            <div className="mm-differs__dashboard-header">
                ...
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                    <span style={{ color: '#00a79e', fontWeight: 600 }}>
                        🟢 {activeCount} Active
                    </span>
            ...
            {/* Summary cards */}
            <div className="mm-differs__stats-grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)', marginBottom: '1.5rem' }}>
                <div className="mm-differs__stat-card">
                    ...
                    <div className="mm-differs__stat-sub">{totalWins}W / {totalLosses}L</div>
                </div>
                <div className="mm-differs__stat-card">
                                ...
                                markets.map(m => (
                                    <tr key={m.marketSymbol} className={m.status === 'stopped' ? 'lost' : ''}>
                                        ...
                                        <td style={{ textAlign: 'center' }}>
                                            <span className={`mm-differs__status-badge ${m.status === 'active' ? 'running' : 'lost'}`}>
                                                {m.status === 'active' ? '● ACTIVE' : '■ STOPPED'}
                                            </span>
                                        </td>
                                        <td style={{ textAlign: 'center', fontWeight: 600 }}>{m.runs}</td>
                                        <td style={{ textAlign: 'center', color: '#00a79e', fontWeight: 600 }}>{m.wins}</td>
                                        <td style={{ textAlign: 'center', color: '#ff444f', fontWeight: 600 }}>{m.losses}</td>
                                        <td style={{ textAlign: 'center' }}>
                                        ...
                                        <td style={{ textAlign: 'right' }}>
                                            <span className={`mm-differs__trade-profit ${m.profit >= 0 ? 'positive' : 'negative'}`}>
                                                {m.profit >= 0 ? '+' : ''}${m.profit.toFixed(2)}
                                            </span>
                                        </td>
                                        <td style={{ textAlign: 'center' }}>
                                            {m.martingaleLevel > 0 ? (
                                                <span className="mm-differs__martingale-badge">Lvl {m.martingaleLevel}</span>
                                            ) : (
                                                <span style={{ color: '#a0a0a0' }}>—</span>
                                            )}
                                        </td>
                        ...
                        {markets.length > 0 && (
                            <tfoot>
                                <tr style={{ borderTop: '2px solid #3e3e3e', background: '#1e1e1e' }}>
                                    <td><strong style={{ color: '#ffcc00' }}>TOTAL</strong></td>
                                    <td style={{ textAlign: 'center' }}>
                                        <span style={{ color: '#a0a0a0', fontSize: '0.85rem' }}>{activeCount} active</span>
                                    </td>
                                    <td style={{ textAlign: 'center', fontWeight: 700, fontSize: '1.1rem' }}>{totalRuns}</td>
                                    <td style={{ textAlign: 'center', color: '#00a79e', fontWeight: 700 }}>{totalWins}</td>
                                    <td style={{ textAlign: 'center', color: '#ff444f', fontWeight: 700 }}>{totalLosses}</td>
                                    <td />
                                    <td style={{ textAlign: 'right' }}>
                                        <strong className={totalProfit >= 0 ? 'positive' : 'negative'} style={{ fontSize: '1.1rem' }}>
                                            {totalProfit >= 0 ? '+' : ''}${totalProfit.toFixed(2)}
                                        </strong>
                                    </td>
                                    <td />
