...
const makeMarket = (symbol: string, name: string): Market => ({
    symbol, name, enabled: false,
    targetDigit: 7, stake: 1, stopLoss: 10, takeProfit: 20,
    martingaleEnabled: false, martingaleMultiplier: 2, martingaleMaxLevels: 5,
});
...
const loadSavedConfig = (): GlobalConfig => {
    ...
    return { targetDigit: 7, stake: 1, stopLoss: 10, takeProfit: 20, martingaleEnabled: false, martingaleMultiplier: 2, martingaleMaxLevels: 5 };
};
...
export const BotConfig = ({ onStartBot, botRunning, onStopBot, errorMessage, connectionStatus }: Props) => {
    ...
    const updateGlobal = (field: keyof GlobalConfig, value: any) => {
        ...
        try { localStorage.setItem(STORAGE_KEY, JSON.stringify(updated)); } catch (e) { /* ignore */ }
        if (mode === 'global') {
    ...
    const totalRisk   = mode === 'global' ? globalConfig.stopLoss * count : enabledMarkets.reduce((s, m) => s + m.stopLoss, 0);
    const totalProfit = mode === 'global' ? globalConfig.takeProfit * count : enabledMarkets.reduce((s, m) => s + m.takeProfit, 0);

    const handleStart = () => {
        if (count === 0) { alert('Please select at least one market'); return; }
                                    ...
                                    <div className="mm-differs__market-settings">
                                        <div className="mm-differs__form-group">
                                            ...
                                            <select
                                                ...
                                                onChange={(e) => updateMarket(index, 'targetDigit', parseInt(e.target.value))}
                                                disabled={botRunning}
                                            >
                                        ...
                                        <div className="mm-differs__form-group">
                                            <label>Stake ($)</label>
                                            <input type="number" value={market.stake} onChange={(e) => updateMarket(index, 'stake', parseFloat(e.target.value))} min="1" disabled={botRunning} />
                                        </div>
                                        <div className="mm-differs__form-row">
                                            <div className="mm-differs__form-group">
                                                <label>SL ($)</label>
                                                <input type="number" value={market.stopLoss} onChange={(e) => updateMarket(index, 'stopLoss', parseFloat(e.target.value))} min="1" disabled={botRunning} />
                                            </div>
                                            <div className="mm-differs__form-group">
                                                <label>TP ($)</label>
                                                <input type="number" value={market.takeProfit} onChange={(e) => updateMarket(index, 'takeProfit', parseFloat(e.target.value))} min="1" disabled={botRunning} />
                                            </div>
                                        </div>
                                        <div className="mm-differs__form-group">
                                            <label>Martingale</label>
                                            <label className="mm-differs__switch">
                                                <input type="checkbox" checked={market.martingaleEnabled} onChange={(e) => updateMarket(index, 'martingaleEnabled', e.target.checked)} disabled={botRunning} />
                                        ...
                                        {market.martingaleEnabled && (
                                            <div className="mm-differs__form-row">
                                                <div className="mm-differs__form-group">
                                                    ...
                                                    <input type="number" value={market.martingaleMultiplier} onChange={(e) => updateMarket(index, 'martingaleMultiplier', parseFloat(e.target.value))} min="1" max="3" step="0.5" disabled={botRunning} />
                                                </div>
                                                <div className="mm-differs__form-group">
                                                    <label>Max Levels</label>
                                                    <input type="number" value={market.martingaleMaxLevels} onChange={(e) => updateMarket(index, 'martingaleMaxLevels', parseInt(e.target.value))} min="1" max="10" disabled={botRunning} />
                                                </div>
                                            </div>
                                        )}
                                    </div>
                ...
                <div className="mm-differs__sidebar">
                    ...
                    {mode === 'global' && (
                        <div className="mm-differs__sidebar-section">
                            ...
                            <div className="mm-differs__form-group">
                                ...
                                <select value={globalConfig.targetDigit} onChange={(e) => updateGlobal('targetDigit', parseInt(e.target.value))} disabled={botRunning}>
                                    {digitOptions.map(d => <option key={d} value={d}>{d}</option>)}
                                </select>
                            </div>
                            ...
                            <div className="mm-differs__form-group">
                                ...
                                <input type="number" value={globalConfig.stake} onChange={(e) => updateGlobal('stake', parseFloat(e.target.value))} min="1" disabled={botRunning} />
                            </div>

                            <div className="mm-differs__form-row">
                                <div className="mm-differs__form-group">
                                    <label>Stop Loss ($)</label>
                                    <input type="number" value={globalConfig.stopLoss} onChange={(e) => updateGlobal('stopLoss', parseFloat(e.target.value))} min="1" disabled={botRunning} />
                                </div>
                                <div className="mm-differs__form-group">
                                    <label>Take Profit ($)</label>
                                    <input type="number" value={globalConfig.takeProfit} onChange={(e) => updateGlobal('takeProfit', parseFloat(e.target.value))} min="1" disabled={botRunning} />
                                </div>
                            </div>
                            ...
                            <div className="mm-differs__form-group">
                                <label>Martingale</label>
                                <label className="mm-differs__switch">
                                    <input type="checkbox" checked={globalConfig.martingaleEnabled} onChange={(e) => updateGlobal('martingaleEnabled', e.target.checked)} disabled={botRunning} />
                                    <span className="mm-differs__slider"></span>
                                </label>
                            </div>
                            ...
                            {globalConfig.martingaleEnabled && (
                                <div className="mm-differs__form-row">
                                    <div className="mm-differs__form-group">
                                        <label>Multiplier</label>
                                        <input type="number" value={globalConfig.martingaleMultiplier} onChange={(e) => updateGlobal('martingaleMultiplier', parseFloat(e.target.value))} min="1" max="3" step="0.5" disabled={botRunning} />
                                    </div>
                                    <div className="mm-differs__form-group">
                                        <label>Max Levels</label>
                                        <input type="number" value={globalConfig.martingaleMaxLevels} onChange={(e) => updateGlobal('martingaleMaxLevels', parseInt(e.target.value))} min="1" max="10" disabled={botRunning} />
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="mm-differs__sidebar-section mm-differs__summary">
                        ...
                        <div className="mm-differs__summary-row">
                            ...
                            <strong className="positive">${(mode === 'global' ? globalConfig.stake : (count > 0 ? totalStake / count : 0)).toFixed(2)}</strong>
                        </div>
                        <div className="mm-differs__summary-row">
                    ...
                    <div className="mm-differs__sidebar-action">
                        ...
                        {errorMessage && (
                            <div className="mm-differs__error-msg">⚠️ {errorMessage}</div>
                        )}
                        {!botRunning ? (
                            <button
                            ...
                            >
                                {connectionStatus === 'connecting' ? '⏳ Connecting...' : `▶ RUN THE BOT${count > 0 ? ` (${count})` : ''}`}
                            </button>
                        ) : (
