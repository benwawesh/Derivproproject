...
const CallbackPage = () => {
    ...
    // Legacy OAuth: tokens arrive directly as URL params (acct1, token1, …)
    ...
    useEffect(() => {
        ...
        // Redirect to the target page WITH token params in the URL so setUserLogin
        // processes them directly — this is the proven Deriv OAuth path.
        const redirect_to = sessionStorage.getItem('redirect_url') || '/';
        sessionStorage.removeItem('redirect_url');
        const token_search = search_params.toString();
        const base = redirect_to.replace(/\?.*$/, ''); // strip any existing query
        window.location.replace(`${base}?${token_search}`);
    }, []);

    if (is_legacy_oauth.current) return null;
