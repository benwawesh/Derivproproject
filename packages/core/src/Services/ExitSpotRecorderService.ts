...
import { batchInsertExitSpots, deleteExitSpot, getExitSpotsForMarket } from './supabase';
...
export class ExitSpotRecorderService {
    ...
    private async flush() {
        ...
        try {
            // Batch insert — backend also auto-prunes each market to CAP=100
            await batchInsertExitSpots(toInsert);
        } catch (e: any) {
...
(window as any).__dpa_fetch_exit_spot = async (
    ...
    // Fetch up to 100 spots for this market from backend, then filter client-side
    let allSpots: Array<{ id: number; epoch: number; exit_price: number }>;
    try {
        allSpots = (await getExitSpotsForMarket(symbol)) as any[];
    } catch {
        return null;
    }
    ...
    // Filter: valid price only — epoch check removed because spots are deleted after use,
    // so epoch-based dedup is unnecessary and can block valid spots when sessionStorage is stale.
    const valid = allSpots.filter(row => row.exit_price != null && !isNaN(Number(row.exit_price)));

    if (valid.length === 0) return null;

    let chosen: { id: number | string; epoch: number; exit_price: number } | undefined;
    ...
    // Consume the row (delete from backend)
    await deleteExitSpot(chosen.id).catch(() => {});
