...
import '../Services/ExitSpotRecorderService'; // auto-starts tick recording on import

import ErrorBoundary from './Components/Elements/Errors/error-boundary.jsx';
import LandscapeBlocker from './Components/Elements/LandscapeBlocker';
import SplashScreen from './Components/SplashScreen';
import DPANavbar from './Components/DPANavbar';
import DPAGlobalTabBar from './Components/DPAGlobalTabBar';
import DPABotTabInjector from './Components/DPABotTabInjector';
import BotXmlLoader from './Components/BotXmlLoader';
...
const AppContent: React.FC<{ passthrough: unknown }> = observer(({ passthrough }) => {
    ...
    React.useEffect(() => {
        const makeHandler = (account_type: 'real' | 'funded') => (e: Event) => {
            const {
                loginid: trade_loginid,
                contract_id,
                market,
                trade_type,
                stake,
        ...
        const realHandler = makeHandler('real');
        const fundedHandler = makeHandler('funded');
        window.addEventListener('dpa_real_trade_completed', realHandler);
        window.addEventListener('dpa_funded_trade_completed', fundedHandler);
        return () => {
            window.removeEventListener('dpa_real_trade_completed', realHandler);
            window.removeEventListener('dpa_funded_trade_completed', fundedHandler);
        };
    }, []);

    /* Trade recording — funded guard handles balance updates directly in DB.
       The dpa_real_trade_completed event above handles real account trade logging. */

    // Exit spot recorder auto-starts on module import (no login required).
    // Nothing to do here — it runs independently of user login state.
    ...
    const [show_splash, setShowSplash] = React.useState(!sessionStorage.getItem('dpa_splash_shown'));

    const handleSplashComplete = React.useCallback(() => {
        sessionStorage.setItem('dpa_splash_shown', '1');
        setShowSplash(false);
    }, []);

    if (show_splash) return <SplashScreen onComplete={handleSplashComplete} />;
    ...
    return (
        <ThemeProvider theme={is_dark_mode_on ? 'dark' : 'light'}>
            ...
            {!isCallBackPage && <DPANavbar />}
            {!isCallBackPage && !location.pathname.startsWith('/bot') && <DPAGlobalTabBar />}
            {!isCallBackPage && <DPABotTabInjector />}
