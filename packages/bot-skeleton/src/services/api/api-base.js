...
class APIBase {
    ...
    async init(force_update = false, loginid = null) {
        if (loginid || getLoginId()) {
            if (loginid) this.pending_account_id = loginid;
    ...
    async createNewInstance(account_id) {
        if (this.account_id !== account_id) {
            this.pending_account_id = account_id;
    ...
    async authorizeAndSubscribe() {
        const { token, account_id } = getToken(this.pending_account_id);
        this.pending_account_id = null;
        if (token) {
            this.token = token;
