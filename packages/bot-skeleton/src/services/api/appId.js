...
export const getToken = override_loginid => {
    const active_loginid = override_loginid || getLoginId();
    const client_accounts = JSON.parse(localStorage.getItem('client.accounts')) || undefined;
    const config_tokens = JSON.parse(localStorage.getItem('config.tokens') || '{}');
    const active_account = (client_accounts && client_accounts[active_loginid]) || {};
    const token = active_account?.token || config_tokens[active_loginid] || undefined;
    return {
        token,
